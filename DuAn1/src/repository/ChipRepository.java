...
public class ChipRepository {
    ...
    public ArrayList<ChipViewModel> getAll() throws SQLException {
        ArrayList<ChipViewModel> listChip = new ArrayList<>();
        String sql = "SELECT MaChip, TenChip FROM CHIP WHERE TrangThai = '0'";
    ...
    public boolean create(ChipViewModel chip) throws SQLException {
        String sql = "INSERT INTO CHIP (MaChip, TenChip, TrangThai) VALUES (?,?,0)";
     ...
     public boolean update(ChipViewModel chip) throws SQLException {
        String sql = "UPDATE CHIP SET TenChip = ? WHERE MaChip =? ";
        try ( Connection conn = DBContext.getConnection();  PreparedStatement stm = conn.prepareStatement(sql);) {
           stm.setString(1, chip.getTen());
           stm.setString(2, chip.getMa());
           stm.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
            return false;
     ...
     public ArrayList<ChipViewModel> getAllXoa() throws SQLException {
        ArrayList<ChipViewModel> listChip = new ArrayList<>();
        String sql = "SELECT MaChip, TenChip FROM CHIP WHERE TrangThai = '1' ";
        Connection conn = DBContext.getConnection();
        PreparedStatement stm = conn.prepareStatement(sql);
        ResultSet rs = stm.executeQuery();
        while (rs.next()) {
     ...
     public boolean updateTT(ChipViewModel chip) throws SQLException {
        String sql = "UPDATE CHIP SET TrangThai = '1' WHERE MaChip =? ";
        try ( Connection conn = DBContext.getConnection();  PreparedStatement stm = conn.prepareStatement(sql);) {
           //stm.setString(1, chip.getTen());
           stm.setString(1, chip.getMa());
           stm.executeUpdate();
