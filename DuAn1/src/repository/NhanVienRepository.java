...
public class NhanVienRepository {
    ...
    public ArrayList<ViewModeNhanVien> getAll() throws Exception {
        ...
        String sql = "SELECT MaNV,Ten,TenDem,Ho, GioiTinh,NgaySinh,DiaChi,SDT,Email, MatKhau,NgayBDLV,GhiChu,ChucVu.Id as IdCv,ChucVu.TenCV as TenCv\n"
    ...
    public Integer them(NhanVien nv) throws Exception {
        ...
        String sql = "INSERT INTO NhanVien\n"
                + "                  (MaNV, Ten, TenDem, Ho, GioiTinh, NgaySinh, DiaChi, SDT, Email, MatKhau, NgayBDLV, GhiChu, Id_ChucVu, TrangThai)\n"
                + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,0)";
        PreparedStatement ps = con.prepareStatement(sql);
        ...
        ps.setString(12, nv.getGhiChu());

        String IdCv = null;
        if (nv.getChucvu() != null) {
            IdCv = nv.getChucvu().getId();
        }
        ps.setString(13, IdCv);
    ...
    public Integer sua(NhanVien nv, String ma) throws Exception {
        ...
        String sql = "UPDATE NhanVien\n"
                + "SET    Ten =?, TenDem =?, Ho =?, GioiTinh =?, NgaySinh =?, DiaChi =?, SDT =?, Email =?, MatKhau =?, NgayBDLV =?, GhiChu =?, Id_ChucVu =?\n"
                + "where MaNV=?";
        ...
        ps.setString(10, nv.getNgayDiLam());
    ...
    public Integer delete1(String ma) throws Exception {
     ...
     public ArrayList<ViewModeNhanVien> timKiem(String ma) throws Exception {
        ...
        String sql = "SELECT MaNV,Ten,TenDem,Ho, GioiTinh,NgaySinh,DiaChi,SDT,Email, MatKhau,NgayBDLV,GhiChu,ChucVu.Id as IdCv,ChucVu.TenCV as TenCv\n"
                + "                                 FROM    NhanVien left join ChucVu on NhanVien.Id_ChucVu=ChucVu.Id   \n"
                + "				where NhanVien.TrangThai=0 and MaNV=?";
        PreparedStatement ps = con.prepareStatement(sql);
        ps.setString(1, ma);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
    ...
    public ArrayList<ViewModeNhanVien> timKiemNhanVienNghiViec(String ma) throws Exception {
        ...
        String sql = "SELECT MaNV,Ten,TenDem,Ho, GioiTinh,NgaySinh,DiaChi,SDT,Email, MatKhau,NgayBDLV,GhiChu,ChucVu.Id as IdCv,ChucVu.TenCV as TenCv\n"
                + "                                 FROM    NhanVien left join ChucVu on NhanVien.Id_ChucVu=ChucVu.Id   \n"
                + "				where NhanVien.TrangThai=1 and MaNV=?";
        PreparedStatement ps = con.prepareStatement(sql);
        ps.setString(1, ma);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
